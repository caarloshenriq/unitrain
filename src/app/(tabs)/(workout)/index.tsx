...
export default function Tab() {
  ...
  const renderItem = ({ item }: { item: Workout }) => (
    <Swipeable
      ...
      onSwipeableWillOpen={() => {
        if (openItem && openItem !== item.workout_id) {
          swipeableRefs.current[openItem]?.close();
        }
        setOpenItem(item.workout_id);
      }}
      onSwipeableWillClose={() => setOpenItem(null)}
      renderRightActions={() => (
    ...
    >
      ...
      >
        <View>
          <Text className="text-black dark:text-white font-bold text-lg">{item.name}</Text>
  ...
  return (
    <View className="bg-white dark:bg-gray-700 dark:color-white p-4 flex-1">
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text className="text-gray-500 text-center mt-10">Nenhum treino cadastrado.</Text>}
      />
    </View>
  );
}
