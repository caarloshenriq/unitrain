...
export default function NewWorkout() {
  ...
  const { getDistinctBodyPart, getExerciseByBodyPart } = useExerciseDatabase(db);
  ...
  function toggleExerciseSelection(exercise_id: number) {
    setSelectedExercises((prevSelected) => {
      const exists = prevSelected.find((item) => item.exercise_id === exercise_id);
      if (exists) {
  ...
  function updateExerciseDetails(exercise_id: number, field: "series" | "repetitions", value: string) {
    setSelectedExercises((prevSelected) =>
  ...
  return (
    <SafeAreaView className="bg-white p-4 flex-1">
      ...
      <View className="border border-gray-300 rounded-md mt-2">
        <Picker selectedValue={weekday} onValueChange={(itemValue) => setWeekday(itemValue)}>
      ...
      <View className="border border-gray-300 rounded-md mt-2">
        <Picker
          ...
          onValueChange={(itemValue) => itemValue && handleBodyPartSelection(itemValue)}
        >
          ...
          <ScrollView className="border border-gray-300 rounded-md mt-2 max-h-64 flex flex-row">
            {exercises.map((exercise) => (
              ...
              >
                ...
                {selectedExercises.find((item) => item.exercise_id === exercise.exercise_id) ? (
  ...
  <>
    ...
    <ScrollView className="border border-gray-300 rounded-md mt-2 max-h-64 p-2">
      {selectedExercises.map((exercise) => (
        <View key={exercise.exercise_id} className="mb-4 border-b border-gray-300 pb-4">
          ...
          <View className="flex-row justify-between mt-2">
            ...
            <View className="w-1/2 pr-2">
              ...
              <TextInput
                ...
                onChangeText={(value) => updateExerciseDetails(exercise.exercise_id, "series", value)}
                placeholder="Ex: 4"
              />
            </View>
            ...
            <View className="w-1/2 pl-2">
              ...
              <TextInput
                ...
                onChangeText={(value) => updateExerciseDetails(exercise.exercise_id, "repetitions", value)}
                placeholder="Ex: 12"
              />
            </View>
          </View>
      ...
      <View className="mt-2">
      <Button title="Salvar Treino" onPress={() => {console.log('receba')}}/>
      </View>
    </SafeAreaView>
  );
}
